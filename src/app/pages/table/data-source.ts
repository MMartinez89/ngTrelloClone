...
export class DataSourceProduct extends DataSource<Product> {
    ...
    find(query: string){
      //  const newProducts = this.originData.filter(item => item.title.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
      //  this.data.next(newProducts);
      const newProducts = this.originData.filter(item =>{
